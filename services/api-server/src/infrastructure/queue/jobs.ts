...
export function setupJobs() {
    ...
    setInterval(async () => {
        try {
            ...
            const { assignments } = await import('@edusphere/types');
            const { eq, and, lte, isNotNull } = await import('drizzle-orm');
            const { assignmentsRepository } = await import('../../modules/assignments/assignments.repository');
            ...
            for (const assignment of pendingReviews) {
                await assignmentsRepository.assignPeerReviews(assignment.id);
            }
            ...
            const { classesService } = await import('../../modules/classes/classes.service');
            ...
            for (const [key, info] of Object.entries(byStudent)) {
                if (info.count >= 3) {
                    const cls = await classesService.getClass(info.classId);
                    if (cls) {
                        await notificationsRepository.createNotification(cls.teacherId, 'wellbeing_alert', `⚠️ Well-being Pattern Detected: A student in ${cls.name} has logged low mood 3+ times in the last 3 days. Consider reaching out.`, `/class/${info.classId}`);
                    }
                }
            }
